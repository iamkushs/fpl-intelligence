...
usage() {
  echo "Usage: $0 [--repo OWNER/REPO] [--wait SECONDS] [--model-route 5.5|luna|terra|sol] [--record-runtime --symphony-ref REF]"
}
...
[[ "$model_route" =~ ^(5\.5|luna|terra|sol)$ ]] || { echo "--model-route must be 5.5, luna, terra, or sol." >&2; exit 2; }
...
while (( SECONDS < deadline )); do
  ...
  has_route="$(printf '%s' "$issue_json" | jq -r --arg route "$model_route" '[.comments[].body | contains("Current model: " + $route) and contains("Actual model ID:")] | any')"
  pr_count="$(gh pr list --repo "$repo" --state open --search "$issue_url in:body" --json number --jq 'length')"
  if [[ "$has_review" == true && "$has_active" == false && "$has_workpad" == true && "$has_route" == true && "$pr_count" -gt 0 ]]; then
